 ...
 */
package week5.actorbintree

import akka.actor._
import week5.actorbintree.BinaryTreeNode.{CopyFinished, CopyTo}
...
class BinaryTreeSet extends Actor with Stash{
  ...
  def garbageCollecting(newRoot: ActorRef): Receive = {
    case msg: Operation => stash() //Leverage akka stash...
    case CopyFinished =>
      root ! PoisonPill
      root = newRoot
...
class BinaryTreeNode(val elem: Int, initiallyRemoved: Boolean) extends Actor {
  ...
  def branch(elem: Int): Position =
    if (elem == this.elem) Middle else if (elem < this.elem) Left else Right
  ...
  val normal: Receive = {
    case op @ Insert(requester, id, el) => findBranch(op) {
      pos => pos match {
