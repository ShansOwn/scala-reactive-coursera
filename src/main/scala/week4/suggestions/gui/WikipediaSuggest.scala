package week4.suggestions.gui

import week4.suggestions.observablex.SchedulerEx
import week4.suggestions.search.Search
...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] =
      searchTerms.sanitized concatRecovered (wikiSuggestResponseStream(_) timedOut 3)
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => pages.observeOn(eventScheduler) subscribe {
        x => x match {
